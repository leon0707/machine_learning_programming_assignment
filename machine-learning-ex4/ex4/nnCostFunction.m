...
r = sigmoid([ones(m,1) sigmoid([ones(m,1) X] * Theta1')] * Theta2');
y_t = eye(num_labels)(y, :);

J = 1/m * sum(sum(-y_t .* log(r) - (1-y_t) .* log(1-r), 2)) + lambda/(2*m)*(sum(sum(Theta1(:,2:end) .^ 2, 2)) + sum(sum(Theta2(:,2:end) .^ 2, 2)));
